...
class AdminMailSettingsController extends EcjiaAdminController
{
    ...
    public function init()
    {
        ...
        $items = \Ecjia\App\Setting\SettingItems::singleton()->getItems('smtp');
    ...
    public function update()
    {
        ...
        RC_Hook::do_action('shop_config_updated_after');

        ecjia_admin_log::instance()->add_object('mail', __('邮件服务器', 'mail'));
        ecjia_admin::admin_log(__('修改邮件服务器设置', 'mail'), 'edit', 'mail');

        return $this->showmessage(__('邮件服务器设置成功。', 'mail'),
            ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS,
            array('pjaxurl' => RC_Uri::url('mail/admin_mail_settings/init'))
        );
    }
