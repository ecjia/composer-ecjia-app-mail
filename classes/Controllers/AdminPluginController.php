...
class AdminPluginController extends AdminBase
{
    public function __construct()
    {
        ...
        RC_Script::enqueue_script('mail_channel', RC_App::apps_url('statics/js/mail_channel.js', $this->__FILE__));
        RC_Script::localize_script('mail_channel', 'js_lang_mail_channel', config('app-mail::jslang.mail_channel'));
    ...
    public function init()
    {
        ...
        ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('邮件渠道', 'mail')));

        $list_data = $this->get_channel_list();
        $this->assign('list', $list_data);

        return $this->display('mail_channel.dwt');
    }
    ...
    public function edit()
    {
        ...
        try {
            ...
            $gourl = RC_Uri::url("mail/admin_plugin/delete", [
                'code' => $channel_code,
                'from' => 'edit',
            ]);
        ...
        return $this->display('mail_channel_edit.dwt');
    }
    ...
    public function delete()
    {
        ...
        return $this->showmessage(__('删除成功', 'mail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('mail/admin_plugin/init')));
    }
    ...
    {
        $this->admin_priv('mail_channel_update', ecjia::MSGTYPE_JSON);
        ...
        if (!empty($id)) {
            ...
            /* 记录日志 */
            ecjia_admin::admin_log($name, 'edit', 'mail_channel');
            return $this->showmessage(__('编辑成功', 'mail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
        } else {
            ...
            /* 记录日志 */
            ecjia_admin::admin_log($name, 'edit', 'mail_channel');
            $refresh_url = RC_Uri::url('mail/admin_plugin/edit', array('code' => $code, 'type' => $type));

            return $this->showmessage(__('安装成功', 'mail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $refresh_url));
        }
    }
    ...
    public function switch_state()
    {
        ...
        ecjia_admin::admin_log($channel_info['channel_name'], $action, 'mail_channel');

        $refresh_url = RC_Uri::url('mail/admin_plugin/init');
        return $this->showmessage(sprintf(__('插件<strong> %s </strong>', 'mail'), $message), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $refresh_url));
    }
    ...
    public function edit_name()
    {
        ...
        $type         = !empty($_GET['type']) ? trim($_GET['type']) : 'mail';
        ...
        ecjia_admin::admin_log(stripcslashes($channel_name), 'edit', 'mail_channel');
        return $this->showmessage(__('编辑成功', 'mail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
    }
    ...
    public function edit_order()
    {
        ...
        if (!is_numeric($_POST['value'])) {
            return $this->showmessage(__('请输入数字类型的排序值', 'mail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
