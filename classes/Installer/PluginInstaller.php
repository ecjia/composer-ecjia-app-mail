...
class PluginInstaller extends \Ecjia\Component\Plugin\Installer\PluginInstaller
{
    ...
    protected function installByCode($code)
    {
        ...
        $data = RC_DB::connection(config('cashier.database_connection', 'default'))->table('notification_channels')->where('channel_type', 'mail')->where('channel_code', $code)->count();
        ...
        if (!$data) {
            ...
            /* 安装，检查该短信插件是否曾经安装过 */
            $count = RC_DB::connection(config('cashier.database_connection', 'default'))->table('notification_channels')->where('channel_type', 'mail')->where('channel_code', $code)->count();
            ...
            if ($count > 0) {
                ...
                RC_DB::connection(config('cashier.database_connection', 'default'))->table('notification_channels')->where('channel_type', 'mail')->where('channel_code', $code)->update($data);
            } else {
